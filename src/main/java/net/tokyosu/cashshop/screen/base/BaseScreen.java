package net.tokyosu.cashshop.screen.base;
...
@OnlyIn(Dist.CLIENT)
public abstract class BaseScreen<T extends AbstractContainerMenu> extends Screen implements MenuAccess<T> {
    ...
    public BaseScreen(T menu, Component playerInvName, Component menuName) {
        super(menuName);
        this.menu = menu;
        this.playerInventoryTitle = playerInvName;
    ...
    protected void renderComponents(@NotNull GuiGraphics pGraphics, int pMouseX, int pMouseY, float pPartialTick) {
        super.render(pGraphics, pMouseX, pMouseY, pPartialTick);
    }

    protected void renderSlots(@NotNull GuiGraphics pGraphics) {
        for(int k = 0; k < this.menu.slots.size(); ++k) {
            Slot slot = this.menu.slots.get(k);
            if (slot.isActive()) {
                this.renderSlot(pGraphics, slot);
            }
        }
    }

    protected void renderHighlightSlots(@NotNull GuiGraphics pGraphics, int pMouseX, int pMouseY) {
        ...
        for (int k = 0; k < this.menu.slots.size(); ++k) {
            ...
            if (this.isHovering(slot, pMouseX, pMouseY) && slot.isActive()) {
                ...
                if (this.hoveredSlot.isHighlightable()) {
                    renderSlotHighlight(pGraphics, l, i1, 0, getSlotColor(k));
                }
            }
        }
    }

    protected void renderDraggedItem(@NotNull GuiGraphics pGraphics, int pMouseX, int pMouseY) {
        ...
        if (!itemstack.isEmpty()) {
            ...
            this.renderFloatingItem(pGraphics, itemstack, pMouseX - this.leftPos - 8, pMouseY - this.topPos - i2, s);
        }
    }

    protected void renderSnapbackItem(@NotNull GuiGraphics pGraphics) {
        if (!this.snapbackItem.isEmpty()) {
            ...
            if (this.snapbackEnd != null) {
                ...
                this.renderFloatingItem(pGraphics, this.snapbackItem, j1, k1, null);
            }
        }
    }
    ...
    public void render(@NotNull GuiGraphics pGuiGraphics, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        this.renderComponents(pGuiGraphics, pMouseX, pMouseY, pPartialTick);

        pGuiGraphics.pose().pushPose();
        pGuiGraphics.pose().translate(this.leftPos, this.topPos, 0.0f);

        this.renderSlots(pGuiGraphics);
        this.renderHighlightSlots(pGuiGraphics, pMouseX, pMouseY);
        this.renderLabels(pGuiGraphics, pMouseX, pMouseY);
        this.renderDraggedItem(pGuiGraphics, pMouseX, pMouseY);
        this.renderSnapbackItem(pGuiGraphics);
    ...
    public boolean mouseClicked(double p_97748_, double p_97749_, int p_97750_) {
        if (super.mouseClicked(p_97748_, p_97749_, p_97750_)) {
            ...
            this.doubleClicked = this.lastClickSlot == slot && i - this.lastClickTime < 250L && this.lastClickButton == p_97750_;
    ...
    public boolean mouseDragged(double p_97752_, double p_97753_, int p_97754_, double p_97755_, double p_97756_) {
        ...
        ItemStack itemstack = this.menu.getCarried();
        if (this.clickedSlot != null && Objects.requireNonNull(this.minecraft).options.touchscreen().get()) {
    ...
    public boolean mouseReleased(double p_97812_, double p_97813_, int p_97814_) {
        ...
        if (this.doubleClicked && slot != null && p_97814_ == 0 && this.menu.canTakeItemForPickAll(ItemStack.EMPTY, slot)) {
            if (hasShiftDown()) {
                if (!this.lastQuickMoved.isEmpty()) {
                    for(Slot slot2 : this.menu.slots) {
                        if (slot2 != null) {
                            ...
                            if (slot2.mayPickup(this.minecraft.player) && slot2.hasItem() && slot2.isSameInventory(slot) && AbstractContainerMenu.canItemQuickReplace(slot2, this.lastQuickMoved, true)) {
